...
- `../../components/lmarena/lmarena_download.py`: Downloads an LM Arena leaderboard snapshot (`overall` category) from the official [lmarena-ai/leaderboard-dataset](https://huggingface.co/datasets/lmarena-ai/leaderboard-dataset) on Hugging Face, and saves it as `lmarena_<subset>_<date>.csv` in the current folder. Run it (from this folder) whenever you want to refresh the scores:
  ```bash
  python3 ../../components/lmarena/lmarena_download.py --subset text_style_control   # default
  python3 ../../components/lmarena/lmarena_download.py --subset webdev
  python3 ../../components/lmarena/lmarena_download.py --subset agent
  ```
...
- `../../components/lmarena/lmarena_scoring.py`: Shared logic (not a script you run directly) for matching OpenRouter model ids to LM Arena entries and computing the cost columns above. Used by both `chat_with_model_choice.py` and `demos/image_analysis`.
