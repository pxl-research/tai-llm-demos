...
with (gr.Blocks(fill_height=True, title='Pixie Lite', css=custom_css) as llm_client_ui):
    ...
    disposal_ico = f'{icons_folder}/disposal.png'
    ...
    with gr.Group(elem_classes='max_height') as gr_live:
        with gr.Row():
            ...
            btn_send_prompt = gr.Button('', scale=0, min_width=64, icon=f'{icons_folder}/send.png')
    ...
    with gr.Group(visible=False) as gr_history:
        with gr.Row():
            ...
            btn_reload_logs = gr.Button(value='', scale=0, min_width=64, icon=f'{icons_folder}/refresh.png')
